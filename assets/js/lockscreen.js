// List of rotating placeholder texts
const placeholders = [
  'Type the word...',
...
function tryLocalFallback(password) {
  ...
  if (url) {
    ...
    setTimeout(() => {
      ...
      // Redirect after fade completes (check 1 second for fade)
      setTimeout(() => {
...
// fixed finally: restore page properly when user presses Back
// fixed finally: lockscreen after redirect doesn't render previous state fresh
...
// Returns a sess ID, generating and storing a new one if needed.
...
async function submitPassword() {
  ...
  // Empty password => throw error
  if (!password) {
    const msg = 'Password cannot be empty!';
    errorMsg.style.color = 'red';
    errorMsg.style.display = 'block';

    // Restart shake animation on error UX
    errorMsg.classList.remove('shake');
    void errorMsg.offsetWidth; // force reflow
    errorMsg.classList.add('shake');

    // smaller delay to sync blink effect
    setTimeout(() => {
    ...
    unlockButton.disabled = false; // Re-enable before returning (should i??)
    return;
  }
  ...
  // tell the user that the password is being processed
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // If fallback fails = backend json error
    ...
    if (data.status === 'master') {
      ...
      // Optional giggle audio = if true = (it's not)
      if (data.playAudio) {
        let audio;
        if (data.audioUrl) {
...
passwordInput.addEventListener('keydown', function (event) {
  ...
  if (event.key === ' ' || event.code === 'Space') {
    ...
    // Submit on Enter = added for UX late (why not? no need of rat)
...
// Sanitize mobile keyboard input where keydown may not trigger
